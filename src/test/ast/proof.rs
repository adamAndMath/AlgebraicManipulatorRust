...
fn double_negate() {
    ...
    assert_eq!(env.truth.get(env.truth.get_id(&Path::parse("DoubleNegate")).unwrap()), &TruthVal::new(Exp::parse("forall<Bool>((b: Bool) -> eq<Bool>(not(not(b)), b))").to_id(&env.local()).unwrap(), 0));
}