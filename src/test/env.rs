...
fn add_data_to_empty_env() {
    let mut data = Vec::<&str>::new();
    {
        ...
        assert_eq!(env.get_id("a"), Some(&0));
        assert_eq!(env.get_id("b"), Some(&1));
        assert_eq!(env.get_id("c"), Some(&2));

        assert_eq!(env.get(0), Some(&"1"));
        assert_eq!(env.get(1), Some(&"2"));
        assert_eq!(env.get(2), Some(&"3"));
    }
...
fn alias_unnamed_data() {
    let mut data = vec!("Not named");
    let mut env = Env::new(&mut data);
    env.alias("name".to_owned(), 0);
    assert_eq!(env.get_id("name"), Some(&0));
    assert_eq!(env.get(0), Some(&"Not named"));
}