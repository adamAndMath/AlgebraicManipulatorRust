...
macro_rules! exp_match {
    ($($($px:ident):*$(::$py:ident)*$(($($pp:tt)*))* => $($ex:ident)::*$([$($eg:tt)*])*$(($($ep:tt)*))*),*) =>
        (vec![$((pattern!($($px):*$(::$py)*$(($($pp)*))*), exp!($($ex)::*$([$($eg)*])*$(($($ep)*))*))),*]);
}

macro_rules! exp_tuple {
    ($($({$($m:tt)*})*$($x:ident):*$(::$y:ident)*$([$($g:tt)*])*$(($($p:tt)*))*)->*) => (exp!($($({$($m)*})*$($x):*$(::$y)*$([$($g)*])*$(($($p)*))*)->*));
    ($($($({$($m:tt)*})*$($x:ident):*$(::$y:ident)*$([$($g:tt)*])*$(($($p:tt)*))*)->*),*) => (Exp::Tuple(vec![$(exp!($($({$($m)*})*$($x):*$(::$y)*$([$($g)*])*$(($($p)*))*)->*)),*]));
}
